    ...
    for indx, filename in enumerate(files_list):
        ...
        with open(filename, 'r') as fobj:
            for line in fobj:
                ...
                # first separate . and ? away from words into seperate lexicons
                line = line.replace('.', ' .')
                line = line.replace('?', ' ?')
                line = line.replace(',', ' ')
...
if __name__ == '__main__':
    ...
    data_dir = join(task_dir, "data\\en-10k\\")
    joint_train = True
    length_limit = None
    files_list = []
    ...
    print("Number of stories with lengths > %d: %d (%% %.2f) [discarded]" % (length_limit, np.sum(stories_lengths > length_limit), np.mean(stories_lengths > length_limit) * 100.0))
