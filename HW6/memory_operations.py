...
def C(M, k, beta):
    ...
    return tf.nn.softmax(similarity * beta, 0)
...
def at(ut, N):
    ...
    # the exclusive argument makes the first element 
    # in the cumulative product a 1 instead of the first element 
    # in the given tensor
    ...
    return full_at_container.stack()


def wwt(ct, at, gw, ga):

    """
    returns the updated write weightings given allocation and content-based
...
def Lt(L, wwt, p, N):
    ...
    def pairwise_add(v):
        ...
        """
        returns the matrix of pairwise-adding the elements of v to themselves
        """
        n = v.get_shape().as_list()[0]
        V = tf.concat([v]*n, 1)  # a NxN matrix of duplicates of u along the columns
        return V + V
